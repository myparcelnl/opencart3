...
class ModelExtensionMyparcelnlShipment extends Model
{
    ...
    {
        $order_query = $this->db->query("SELECT export_settings FROM `" . DB_PREFIX . self::$table_name . "` WHERE order_id = " . (int)$order_id . " LIMIT 1");
    ...
    {
        $order_query = $this->db->query("SELECT delivery_options, signed, recipient_only FROM `" . DB_PREFIX . self::$table_name . "` WHERE order_id = " . (int)$order_id . " LIMIT 1");
    ...
    {
        $order_query = $this->db->query("SELECT shipment_data FROM `" . DB_PREFIX . self::$table_name . "` WHERE order_id = " . (int)$order_id . " LIMIT 1");
    ...
    {
        $order_query = $this->db->query("SELECT signed FROM `" . DB_PREFIX . self::$table_name . "` WHERE order_id = " . (int)$order_id . " LIMIT 1");
    ...
    {
        $order_query = $this->db->query("SELECT extra_options FROM `" . DB_PREFIX . self::$table_name . "` WHERE order_id = " . (int)$order_id . " LIMIT 1");
    ...
    public function update($order_id, $column_name, $value)
    {
        ...
        if ($query->num_rows) {
            $this->db->query("UPDATE " . DB_PREFIX . self::$table_name . " SET " . $column_name . "='" . $data . "' WHERE order_id =" . (int)$order_id);
        } else {
            $this->db->query("INSERT INTO " . DB_PREFIX . self::$table_name . " SET order_id = " . (int)$order_id . ", " . $column_name . "='" . $data . "'");
        }
    }
