    ...
    <tr>
        ...
        <td>
            ...
            if(isset($recipient['cc']) && $recipient['cc'] != 'NL'){
                unset($data['package_types'][4]);//digital stamp
            }
            if ($is_pickup) {
                unset($data['package_types'][2]); // mailbox package
                unset($data['package_types'][3]); // unpaid letter
                unset($data['package_types'][4]); // degital stamp
...
<table class="ocmyparcel_settings_table digital_stamp_options">
    <tbody>
    <tr>
        <td>
            <?php echo $data['entry_weight'] ?>
        </td>
        <td >
            <select name="myparcel_options[<?php echo $order_id; ?>][digital_stamp_weight]" >
                <?php foreach ($data['digital_stamp_weights'] as $key => $weight) : ?>
                    <?php $selected = (isset($myparcel_options[$order_id]['digital_stamp_weight'])) ? 'selected="selected"' : '' ; ?>
                    <option <?php echo $selected ?> value="<?php echo $key ?>"><?php echo $weight ?></option>
                <?php endforeach; ?>
            </select>
        </td>
    </tr>
